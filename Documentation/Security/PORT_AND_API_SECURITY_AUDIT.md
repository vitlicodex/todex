...
Remaining work is mostly product hardening: explicit redirect policy, granular API errors, OpenAI API pagination, stricter privacy mode for project labels, local estimated cost modeling, Developer ID signing, hardened runtime, and notarization.
...
- Costs API data can be available even when Usage API fails, so partial-failure status must stay visible in UI.
...
| TDX-SEC-004 | Medium | OpenAI API outbound | `Sources/TokenUsageCore/OpenAIUsageClient.swift:316` | Authorization header could be sent to a non-HTTPS custom base URL if a developer/test setting changed the endpoint. | Misconfigured build points Usage API client to `http://...` and sends Bearer token in cleartext. | `requestJSON` now rejects non-HTTPS URLs before creating the Authorization request. | Add host redirect pinning and mock URLProtocol tests. | Build/test runner; no network smoke test. | Fixed |
...
| TDX-SEC-007 | Medium | OpenAI API errors | `Sources/TokenUsageCore/OpenAIUsageClient.swift:335` | 429, 5xx, endpoint-unavailable, and timeout errors are collapsed into generic failures. | UI shows vague warning and hides whether retry/backoff or key permission action is needed. | 401/403 are separate, but other HTTP failures are generic sanitized strings. | Add typed errors for 429, Retry-After, 5xx, timeout, endpoint unavailable, and permission scope missing. | Existing API parse tests only. | Planned |
| TDX-SEC-008 | Medium | OpenAI API pagination/redirects | `Sources/TokenUsageCore/OpenAIUsageClient.swift:267`, `Sources/TokenUsageCore/OpenAIUsageClient.swift:316` | Missing pagination can undercount; redirects are not explicitly host-pinned. | Usage/Costs API returns additional pages or redirects to an unexpected host. | Query limit is fixed at 62 daily buckets and `URLSession` default redirect behavior is used. | Implement cursor pagination and `URLSessionTaskDelegate` redirect validation. | Not covered yet. | Planned |
...
| TDX-SEC-010 | Medium | Token/cost accounting | `Sources/TokenUsageCore/TokenUsageParser.swift:560`, `Sources/TokenUsageCore/TokenUsageStore.swift:250` | Misleading averages/costs can cause bad budget decisions. | User treats local Codex tokens as actual OpenAI Costs API billing or treats request average as user prompt average. | Parser counts `last_token_usage`, preserves cached input, but reasoning/reported totals and estimated local cost are not fully modeled. | Add reported/computed/reasoning fields and an explicit estimated local cost engine separated from actual Costs API. | Parser/accounting tests cover cached input and UI model basics. | Planned |
...
| TDX-SEC-013 | Low | Help WebView | `Sources/TokenUsageMenuBar/HelpWindowController.swift` | Future help links could navigate outside local bundled content. | A future Markdown change adds remote links and WebView opens them inside app context. | Current help is local bundled Markdown with escaped HTML, but navigation policy is not explicitly locked down. | Add navigation delegate that only allows bundled file URLs and externalizes web links to the browser. | Not covered yet. | Planned |
